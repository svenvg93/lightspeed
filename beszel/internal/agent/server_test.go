...
// createTestCombinedData creates test data for encoding tests
func createTestCombinedData() *system.CombinedData {
	return &system.CombinedData{
		...
		Stats: system.Stats{
			PingResults: map[string]*system.PingResult{
				"test-host": {
					Host:        "test-host",
					PacketLoss:  0,
					MinRtt:      1.0,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedUsesCbor {
				...
				assert.Equal(t, len(testData.Stats.PingResults), len(decodedCbor.Stats.PingResults))
			} else {
				...
				assert.Equal(t, len(testData.Stats.PingResults), len(decodedJson.Stats.PingResults))
