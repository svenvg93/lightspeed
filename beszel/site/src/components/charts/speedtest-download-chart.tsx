...
interface SpeedtestDownloadChartProps {
	chartData: ChartData
	serverIds: string[]
			...
			<div>
				...
				>
					<ComposedChart accessibilityLayer data={speedtestData} margin={chartMargin}>
						...
						{/* Download Speed Areas for each server */}
						{serverIds.map((serverId: string, index: number) => (
							<Area
								key={serverId}
								yAxisId="left"
								dataKey={(data) => data[serverId]?.download_speed ?? null}
								name={serverNames?.[serverId] || serverId}
								type="monotoneX"
								fill={`url(#fillDownloadSpeed-${serverId})`}
								fillOpacity={0.4}
