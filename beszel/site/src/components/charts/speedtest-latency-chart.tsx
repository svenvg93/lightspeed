...
interface SpeedtestLatencyChartProps {
	chartData: ChartData
	serverIds: string[]
	serverNames?: Record<string, string> // Map of server ID to friendly name
}

export default function SpeedtestLatencyChart({ chartData, serverIds, serverNames }: SpeedtestLatencyChartProps) {
			...
			<div>
				...
				>
					<ComposedChart accessibilityLayer data={speedtestData} margin={chartMargin}>
						...
						{/* Latency Areas for each server */}
						{serverIds.map((serverId, index) => (
							<Area
								key={serverId}
								yAxisId="left"
								dataKey={(data) => data[serverId]?.latency ?? null}
								name={serverNames?.[serverId] || serverId}
								type="monotoneX"
								fill={`url(#fillLatency-${serverId})`}
								fillOpacity={0.4}
