...
interface SpeedtestUploadChartProps {
	chartData: ChartData
	serverIds: string[]
	serverNames?: Record<string, string> // Map of server ID to friendly name
}

export default function SpeedtestUploadChart({ chartData, serverIds, serverNames }: SpeedtestUploadChartProps) {
			...
			<div>
				...
				>
					<ComposedChart accessibilityLayer data={speedtestData} margin={chartMargin}>
						...
						{/* Y-axis for Upload Speed */}
						<YAxis
							yAxisId="left"
							orientation="left"
							className="tracking-tighter"
							width={yAxisWidth}
							tickFormatter={(value) => updateYAxisWidth(toFixedFloat(value, 1) + " Mbps")}
							tickLine={false}
							axisLine={false}
						/>
						{xAxis(chartData)}
							...
							content={
								<ChartTooltipContent
									...
									contentFormatter={({ value }) => {
										return decimalString(value, 1) + " Mbps"
									}}
								/>
							}
						/>
						...
						{/* Upload Speed Areas for each server */}
						{serverIds.map((serverId: string, index: number) => (
							<Area
								key={serverId}
								yAxisId="left"
								dataKey={(data) => data[serverId]?.upload_speed ?? null}
								name={serverNames?.[serverId] || serverId}
								type="monotoneX"
								fill={`url(#fillUploadSpeed-${serverId})`}
								fillOpacity={0.4}
