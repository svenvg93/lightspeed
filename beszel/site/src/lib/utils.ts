	...
	return async () => {
		...
		try {
			const records = await pb
				...
				.getFullList({ sort: "+name", fields: "id,name,host,info,status,monitoring_config,averages,tags" })
...
export const chartMargin = { top: 8, left: 8, right: 8 }
...
export function parseCronInterval(cronExpression: string): number | null {
	...
	// For complex cron expressions, we can't easily determine the interval
	// Return null to use a default fallback
	return null
}
