...
func TestSystemManagerNew(t *testing.T) {
	...
	t.Run("concurrent", func(t *testing.T) {
		...
		time.Sleep(10 * time.Millisecond) // Allow goroutines to execute
...
func testOld(t *testing.T, hub *tests.TestHub) {
	...
	t.Run("HandleSystemData", func(t *testing.T) {
		...
		testData := &system.CombinedData{
			...
			Stats: system.Stats{
				PingResults:      make(map[string]*system.PingResult),
				DnsResults:       make(map[string]*system.DnsResult),
				HttpResults:      make(map[string]*system.HttpResult),
				SpeedtestResults: make(map[string]*system.SpeedtestResult),
			},
		}
